...
const saveLibraryData = () => {
  fs.writeFileSync(
    libraryDataPath,
    JSON.stringify(Array.from(libraryData.entries()), null, 2)
  );
};
...
const issueBook = async (rl) => {
  try {
    ...
    const bookTitleInput = await askQuestion(rl, "Enter the book to issue: ");
    ...
    if (foundBook) {
      ...
      const response = await askQuestion(
        rl,
        "Do you want to issue this book? (yes/no): "
      );
      ...
      if (response.toLowerCase() === "yes") {
        if (foundBook.quantity > 0) {
          ...
          let userRecord = issuedBooksData.records.find(
            (record) =>
